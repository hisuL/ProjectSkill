...
description: Use when converting a PRD into an architecture design document through staged analysis and user-confirmed architecture decisions, before writing architecture content / 用于基于 PRD 分阶段产出架构设计文档，在正式写文档前先完成分析和关键架构决策确认
user-invocable: true
context: fork
agent: Plan
---
...
- `PRD`：产品需求文档，是架构设计的业务输入
- `research`：调研材料，补充技术候选、约束和参考案例
- `domain`：业务域，强调业务职责如何分组
- `service`：服务，强调系统如何部署或运行
- `module`：模块，强调后续技术设计消费边界
- `module_id`：模块唯一标识，供 `technical-design` 作为目录名和消费键使用
- `work item`：工作项，强调交付拆分，不等于模块边界
- `handoff`：交接信息，指要交给后续技术设计继续消费的稳定内容
- `trade-off`：权衡，指不同方案之间的收益和代价比较
- `core flow`：核心流程，指最影响系统架构的主路径
- `failure path`：异常路径，指失败、重试、补偿、降级等路径
...
1. **Enter Plan mode / 进入计划模式** - 架构设计必须先规划，不能一口气直接写完
2. **Read source context / 读取主输入** - 读取 `docs/01-prd/PRD.md` 和 `docs/01-prd/research.md`
3. **Expand referenced material / 展开引用材料** - 扫描图片、markdown 和其他引用文档并读取
4. **Summarize architecture inputs / 总结架构输入** - 列出引用文件和图片，并说明各自影响
5. **Identify business domains / 识别业务域** - 提出 2-3 种划分方案，解释权衡并与用户确认
6. **Design service/module boundaries / 设计服务与模块边界** - 提出聚合方案，比较粗细粒度并确认
7. **Narrow technical choices / 收敛技术选型** - 从 research 提炼候选技术，比较 2-3 个选项并确认
8. **Design core flows / 设计核心流程** - 梳理主流程和异常路径，并与用户确认
...
12. **Sync work items to PingCode / 同步工作项到 PingCode** - 当用户已指定 PingCode 项目或明确要求同步时，使用 `pingcode-skill` 将工作项清单的新增和变更同步到 PingCode，并把工作项标识回写文档；架构阶段创建的工作项类型必须是“用户故事 / story”，禁止创建为 `task`
...
4. 如果用户提供了 PingCode 项目上下文或明确要求同步，则调用 `pingcode-skill`：
   ...
   - 新创建的 PingCode 工作项类型必须使用“用户故事 / story”，不要使用 `task`
   - 保持这些架构阶段工作项可在后续编码阶段继续拆分为任务或子任务，不要在架构阶段提前消耗掉拆分层级
...
7. 如果本次更新影响 `工作项清单`，且用户已要求同步 PingCode，则必须使用 `pingcode-skill` 同步新增和变更：
   - 没有 PingCode 标识的工作项视为新增，创建到目标项目，且工作项类型必须为“用户故事 / story”
   - 已有 PingCode 标识的工作项，如标题或职责描述变更，则更新到对应工作项
   - 如果发现历史上被错误创建为 `task`，应先提醒用户该工作项已失去后续拆分层级，再根据用户指令决定是否迁移或重建为“用户故事 / story”
...
- 这些由架构阶段同步到 PingCode 的工作项，默认语义是“用户故事”而不是“任务”；这样后续编码阶段仍可继续按实现、联调、测试拆分子任务
...
- 如果用户要求同步到 PingCode，创建时默认把这些工作项映射为“用户故事 / story”，不要映射为 `task`
